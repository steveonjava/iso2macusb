...
`dd`-ing the Proxmox hybrid ISO to a USB stick and booting it on some Apple firmware fails. It
drops to a `grub>` prompt, and `linux /boot/linux26` returns **`error: invalid magic number`**.
...
prompt with **`error: file '/x86_64-efi/ls.mod' not found`** because its `prefix` is empty or
wrong, so it can't load runtime `.mod` files or even its own `grub.cfg`, and `search --label`
...
natively, GRUB loads the kernel and initrd as plain files, and the Proxmox auto-installer runs.
...
- **Stage 1, `prepare-iso.sh`**: bakes your `answer.toml` into a stock Proxmox ISO via
  `proxmox-auto-install-assistant`. The default `--fetch-from iso` is fully offline (embedded
  initrd), which is best for Macs with no reliable installer-time NIC. Needs the Proxmox repos, or
  a Debian-13 host or container with `proxmox-auto-install-assistant`.
...
Identify the USB unambiguously first with `lsblk -o NAME,SIZE,TYPE,TRAN,MODEL,SERIAL`, then pick
the one with `TRAN=usb` and `removable=1`. **Double-check the device; `dd` to the wrong disk is destructive.**

## Booting on the Mac

1. Insert the USB in a **rear/direct port** (avoid hubs and front ports). Power on; hold **Option (⌥)**
   if needed and pick the EFI Boot volume.
2. GRUB shows a 2-entry menu and auto-selects entry 1 after 5s:
   `Loading kernel...` then `Loading initrd (...)` (a pause if the initrd is large), then the Proxmox
...
- If you built the ISO with **`--fetch-from iso`** (the *embedded-ISO* variant), the initrd is
  large (~1.7 GB) and contains the whole installer payload, so the install is fully offline. This is
  the **recommended** setup for a Mac with no reliable installer NIC at boot.
- If you used the default prepared ISO, `/boot/initrd.img` is small (~55 MB) and the installer
  expects to **fetch the rest over the network/HTTP**, so make sure that path works on the target.
...
**Do not commit secrets**: keep `answer.toml`, password hashes, SSH keys, and internal IPs or
hostnames out of the published ISO/image and out of this repo.
